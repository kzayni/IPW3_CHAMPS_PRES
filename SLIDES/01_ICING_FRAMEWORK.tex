...
Droplet Drag Model &
\multicolumn{2}{c|}{Deformable Sphere \cite{clift2005bubbles}} \\
\hline
...
\begin{flushleft}
\tiny
\textbf{\textcolor{red}{*}} Flow equations use Roe convective fluxes with the
...
\textbf{0-level set} of a signed-distance function $\phi$
\cite{OSHER2004,FROLKOVIC2015214,lavoieLevelSet, alessandroLevelSet}:
...
\begin{itemize}
    ...
    \item $\boldsymbol{n}_{\phi_0}$ is computed from the initial wall-distance field $\phi_0$.

    \item The predicted ice thickness defines the interface velocity $\boldsymbol{V}$.

    \item The interface is advected through unsteady time stepping until the prescribed icing time or ice volume is reached.

    \item The $\boldsymbol{\phi=0}$ interface is extracted using Mmg \cite{dapogny2014mmg}, and the computational mesh is regenerated using automated Pointwise scripts.

    \item \textbf{Current caveats:} dedicated level-set meshes are required to avoid restrictive time steps associated with near-wall RANS refinement, and the Mmg-based interface extraction requires a triangular surface mesh.
\end{itemize}
